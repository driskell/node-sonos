...
Listener.prototype._messageHandler = function (req, res) {
  if (req.method.toUpperCase() === 'NOTIFY' && req.url.toLowerCase() === '/notify') {
    ...
    this.parser.parseString(req.body.toString(), function (error, data) {
      ...
      this.emit('serviceEvent', thisService.endpoint, req.headers.sid, thisService.data, thisService.device)
      res.end()
    }.bind(this))
  }
}
...
Listener.prototype._renewServices = function () {
  ...
  var renew = function (sid) {
    return function (err, response) {
      var serviceEndpoint = this.services[sid].endpoint,
          device = this.services[sid].device
  ...
  for (sid in this.services) {
    ...
    var opt = {
      url: 'http://' + thisService.device.host + ':' + thisService.device.port + thisService.endpoint,
      method: 'SUBSCRIBE',
      headers: {
        SID: sid,
        Timeout: 'Second-3600'
...
Listener.prototype.addService = function (serviceEndpoint, callback, device) {
  if (!this.server) {
    throw new Error('Service endpoints can only be added after listen() is called')
  }

  var thisDevice = device || this.device;

  var opt = {
    url: 'http://' + thisDevice.host + ':' + thisDevice.port + serviceEndpoint,
    method: 'SUBSCRIBE',
    headers: {
...
Listener.prototype.removeService = function (sid, callback, device) {
  if (!this.server) {
  ...
  thisDevice = device || this.device;

  if (this.services[sid].device != thisDevice) {
    throw new Error('Service with sid ' + sid + ' is not registered to given device')
  }

  var opt = {
    url: 'http://' + thisDevice.host + ':' + thisDevice.port + this.services[sid].endpoint,
    method: 'UNSUBSCRIBE',
    headers: {
      sid: sid
    }
